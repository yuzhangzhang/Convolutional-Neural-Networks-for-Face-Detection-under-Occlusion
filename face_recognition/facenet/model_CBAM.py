...
# from Mask_bce_loss import Attention_loss
import numpy as np
# from Mask_bce_loss import UnNormalizer
    ...
    def forward(self, inputs):
        ...
        if self.showlayer:
            ...
            for level in [x]:
                ...
                # plt.imsave(os.path.join(pwd, 'week7/Layer_show', version + '.jpg'), level[:, :, 0])
                # for ij in range(layers):
                #     plt.imsave(os.path.join(pwd, 'week7/Layer_show', version+ '_layer%s.jpg'%ij), level[:, :, ij])
        # print(x)
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
