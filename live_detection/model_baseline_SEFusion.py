import os
from live_detection.utils import *
...
from live_detection.model_baseline import Net
from live_detection.FaceBagNet import SEModule
...
class FusionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # fea = torch.cat([color_feas, depth_feas, ir_feas], dim=1)   #channel维度，使得c=128x3=384 ------

        x = self.res_0(color_feas)        #-----------
        x = self.res_1(x)
        x = F.adaptive_avg_pool2d(x, output_size=1).view(batch_size, -1)   #(batch_size, channel, H, W)
        x = self.fc(x)
        return x,None,None
